...
class LSTMRedModel(nn.Module):
    def __init__(self, input_size, embedding_size, hidden_size, num_classes, num_layers=2, dropout=0.5):
        super(LSTMRedModel, self).__init__()

        self.hidden_size = hidden_size
        self.num_layers = num_layers
        self.embedding = nn.Embedding(num_classes, embedding_size)
        self.lstm = nn.LSTM(embedding_size * input_size, hidden_size, num_layers=num_layers, 
                            batch_first=True, dropout=dropout)
